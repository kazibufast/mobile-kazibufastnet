...
export default function MpinLoginScreen() {
  ...
  const [phoneNumber, setPhoneNumber] = useState(""); // ✅ stateful phone number
  ...
  // ✅ Load phone number from params or AsyncStorage
  useEffect(() => {
    const loadPhoneNumber = async () => {
      const paramPhone = params.phone as string | undefined;
      if (paramPhone) {
        setPhoneNumber(paramPhone);
      } else {
        const savedPhone = await AsyncStorage.getItem("verifiedPhoneNumber");
  ...
  const loginRequest = async (mobile_number: string, pin: string) => {
    try {
        ...
        try {
            ...
            console.warn("Failed to parse JSON, using text as message");
        ...
        if (response.ok && data.token) {
            ...
            await AsyncStorage.setItem('verifiedPhoneNumber', mobile_number); // ✅ save phone number
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () => {
            Alert.alert(
              "Contact Support",
              "Please call 1-800-123-4567 for assistance."
            );
          },
        },
      ]
  ...
  const handleChangeNumber = () => {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        { text: "Yes", onPress: () => router.push("/index") },
      ]
    );
  };
