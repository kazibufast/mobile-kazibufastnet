...
export default function TimeInScreen() {
  ...
  const handleTimeIn = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://tub.kazibufastnet.com/api/tech/time_in/${technician?.id}`, {
        method: 'POST',
        headers: { 
      ...
      if (res.ok) {
        Alert.alert('Success', 'Time In recorded successfully!', [
          { text: 'OK', onPress: () => router.push('/(tech-tabs)/home') }
        ]);
      } else {
        const err = await res.json();
        console.error(err);
        Alert.alert('Error', 'Failed to submit Time In. Please try again.');
      }
    } catch (error) {
      console.error(error);
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    <Text style={styles.actionButtonText}>Submit Time In</Text>
                  </>
                )}
              </TouchableOpacity>
            </View>
