...
const Ticket: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'open' | 'pending' | 'accepted' | 'inProgress' | 'completed'>('all');
  ...
  const getStatusConfig = (status: TicketItem['status']) => {
    switch (status) {
      ...
      case 'Pending':
        return { color: '#D97706', bgColor: '#FEF3C7', icon: 'hourglass-outline' };
      case 'InProgress':
        return { color: '#D97706', bgColor: '#f0dac2ff', icon: 'hourglass-outline' };
      case 'Accepted':
        return { color: '#464646ff', bgColor: '#c7daf0ff', icon: 'checkmark-circle-outline' };
      case 'Completed':
        return { color: '#059669', bgColor: '#D1FAE5', icon: 'checkmark-done-circle-outline' };
      case 'Closed':
        return { color: '#6B7280', bgColor: '#F3F4F6', icon: 'lock-closed-outline' };
      default:
        ...
        <View style={styles.headerSection}>
          ...
          >
            {[
              ...
              { key: 'inProgress', label: 'In Progress', count: tickets.filter(t => t.status === 'InProgress').length },
              { key: 'completed', label: 'Completed', count: tickets.filter(t => t.status === 'Completed' || t.status === 'Closed').length },
            ].map((tab) => (
              <TouchableOpacity
                key={tab.key}
                style={[
                          ...
                          <View style={styles.headerRow}>
                            ...
                            {ticket.type && (
                              <View style={styles.typeBadge}>
                                ...
                                />
                                <Text
                                  style={[styles.typeText, { color: typeConfig.color }]}
                                  numberOfLines={1}
                                >
                                  {ticket.type}
                                </Text>
