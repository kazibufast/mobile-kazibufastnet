...
export default function LoginScreen() {
  ...
  useEffect(() => {
    const loadSavedPhone = async () => {
      ...
      if (savedPhone) {
        ...
        setShowOtpScreen(true); // optionally show OTP screen if desired
      }
    };
    loadSavedPhone();
  }, []);
  ...
  const handleSendOtp = async () => {
    ...
    setTimeout(() => {
      ...
      Alert.alert(
        "OTP Sent",
        `Verification code sent to ${encryptPhoneNumber(cleanPhone)}`
      );
    }, 1500);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // Save verified phone to AsyncStorage
      await AsyncStorage.setItem("verifiedPhone", phoneNumber);

      Alert.alert("Success", "Phone number verified successfully!");
      router.push({
        pathname: "/setup-pin",
        params: { phone: "+63" + phoneNumber, verified: "true" },
      });
    } catch (error: any) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      Alert.alert(
        "OTP Resent",
        `A new verification code has been sent to ${encryptPhoneNumber(
          cleanPhone
        )}`
      );
    } catch (error: any) {
  ...
  const handleSignIn = () => router.push("/mpin-login");
        ...
        <View style={styles.container}>
          <View style={styles.logoContainer}>
            ...
            <View style={{ flexDirection: "row" }}>
              ...
              <Text
                style={{ fontSize: 24, fontWeight: "900", color: "#00afa1ff" }}
              >
                FAST
              </Text>
            </View>
