...
export default function TimeTrackerScreen({ type }: TimeTrackerScreenProps) {
  ...
  const submitLabel = isTimeIn ? "Submit Time In" : "Time Out";
  const successMessage = isTimeIn
    ? "Time In recorded successfully!"
    : "Time Out recorded successfully!";
  const errorMessage = isTimeIn
    ? "Failed to submit Time In. Please try again."
    : "Failed to submit Time Out. Please try again.";
  const userStatus = isTimeIn ? "active" : "out";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const apiUrl = isTimeIn ? API.tech.timeIn() : API.tech.timeOut();
      ...
      if (res.ok) {
        ...
        if (currentUser) {
          const updated = { ...currentUser, status: userStatus };
          setUser(updated);
          await AsyncStorage.setItem("user", JSON.stringify(updated));
        }

        if (!isTimeIn) {
          const { setSkipAttendanceCheck } = require("../app/(tech-tabs)/_layout");
      ...
      >
        ...
        {showCamera ? (
          ...
          <View style={styles.photoContainer}>
            ...
            <Image
              source={{
                uri: photo.base64.startsWith("data:")
                  ? photo.base64
