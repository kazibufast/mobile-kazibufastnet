...
// Storage keys
const STORAGE_KEYS = {
  MESSAGES: '@kazichat_messages',
  SUGGESTED_QUESTIONS: '@kazichat_suggested_questions',
...
const OverlayButton: React.FC = () => {
  ...
  // MOVE THIS LINE INSIDE THE COMPONENT (Fix the error)
  ...
  useEffect(() => {
    const loadStoredData = async () => {
      try {
        ...
        const storedQuestions = await AsyncStorage.getItem(STORAGE_KEYS.SUGGESTED_QUESTIONS);
        const storedAttachments = await AsyncStorage.getItem(STORAGE_KEYS.ATTACHMENTS);
        ...
        if (storedQuestions) {
          setSuggestedQuestions(JSON.parse(storedQuestions));
        }
  ...
  // Save suggested questions to storage whenever they change
  useEffect(() => {
    if (!isInitializing) {
      const saveQuestions = async () => {
        try {
          await AsyncStorage.setItem(STORAGE_KEYS.SUGGESTED_QUESTIONS, JSON.stringify(suggestedQuestions));
        } catch (error) {
  ...
  }, [suggestedQuestions, isInitializing]);

  // Save attachments to storage whenever they change
  useEffect(() => {
    if (!isInitializing) {
  ...
  // Clear all stored data (optional - for testing or logout)
  const clearStoredData = async () => {
    try {
      await AsyncStorage.removeItem(STORAGE_KEYS.MESSAGES);
      await AsyncStorage.removeItem(STORAGE_KEYS.SUGGESTED_QUESTIONS);
      await AsyncStorage.removeItem(STORAGE_KEYS.ATTACHMENTS);
      setMessages([
        {
          id: '1',
          text: "Hello! I'm here to help you create a support ticket. You can describe your issue and attach photos or files if needed.",
  ...
  // Add a clear history button function
  ...
  useEffect(() => {
    // Scroll to bottom only when shouldAutoScroll is true
    if (flatListRef.current && shouldAutoScroll && messages.length > 0) {
      setTimeout(() => {
